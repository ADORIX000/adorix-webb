      ...
      <div className="flex items-center justify-center px-4 py-20 min-h-screen">
        <div className="w-full max-w-[420px] bg-white rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.07)] border border-gray-100 p-8">
          ...
          <form onSubmit={onPressVerify}>
            <div className="mb-4">
              <input value={code} onChange={(e) => setCode(e.target.value)} placeholder="Verification Code" className="w-full h-11 px-4 bg-white border border-gray-200 rounded-md text-sm text-[#1F2B2D] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#0D8A9E]/30 focus:border-[#0D8A9E] transition-all" required />
            </div>
            <button type="submit" disabled={loading} className="w-full h-10 bg-[#0D8A9E] hover:bg-[#0a7a8d] active:bg-[#085a66] text-white text-sm font-semibold rounded-lg transition-all duration-150 shadow-none mt-2 flex items-center justify-center gap-1.5 disabled:opacity-75 disabled:cursor-not-allowed">
              Verify
            </button>
          </form>
        </div>
